...
app.get('/', function(req, res){
    res.sendFile(path.join(__dirame + '/pr11.html'));
});
...
wss2.on('connection', function(ws, req){
    ...
    ws.on('message', function(msgString){ // sporočilo dobimo na stran strežnika kot string, t.j. msgString
        ...
        switch(msg.tipSporočila){
            case "tipka":
                funkcijaTipka(msg); // v primeru, da je tip sporočila iz Chrome klient "LED" poženemo funkcijo funkcijaLED
            break;
        }
...
function funkcijaLED(msg){ // če je tip sporočila LED
    wss2.broadcast(JSON.stringify(msg)); // "broad-castamo" po vseh esp-jih - JSON sporočilo posredujemo vsem esp32 modulom
}
